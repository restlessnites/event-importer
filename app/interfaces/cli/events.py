...
def show_event_details(args) -> None:
        ...
        if event.submissions:
            ...
            for sub in event.submissions:
                submission_data.append({
                    ...
                    "Submitted": sub.submitted_at.strftime("%Y-%m-%d %H:%M") if sub.submitted_at else "N/A",
                    "Retries": sub.retry_count,
                    "Error": sub.error_message[:100] + "..." if sub.error_message and len(sub.error_message) > 100 else sub.error_message or "N/A"
                })
            cli.table(submission_data)
        else:
            cli.info("No submissions for this event")
...
def show_stats(args) -> None:
    ...
    if submission_stats["total_submitted_events"] > 0:
        ...
        for status, count in submission_stats["by_status"].items():
            submission_data.append({
                "Metric": f"{status.title()} Submissions", 
                "Count": count
            })
