...
async def test_ra_genres() -> None:
    ...
    await _test_individual_events(cli, http)
    await _test_event_listings(cli, http)
    await _test_genre_list(cli, http)
    _print_summary(cli)
...
async def _test_individual_events(cli, http):
...
async def _test_event_listings(cli, http):
    ...
    try:
        ...
        cli.success(
            f"Found {len(events_with_genres)} events with genres out of {len(listings)}"
        )
...
async def _test_genre_list(cli, http):
